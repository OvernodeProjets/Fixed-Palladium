...
router.get("/delete", ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const server = await axios.get(
      `${provider.url}/api/application/servers/${serverId}`,
      {
        headers: {
    ...
    logToDiscord(
      ...
      `${req.user.username} has deleted server with : \n\`\`\`Name: ${
        ...
        server.data.attributes.limits.ram
      } MB\nDisk: ${server.data.attributes.limits.disk} MB\nBackup: ${
        server.data.attributes.feature_limits.backups || 0
      }\nDatabase: ${
        server.data.attributes.feature_limits.database || 0
      }\nAllocation: ${
        server.data.attributes.feature_limits.allocation || 0
      }\`\`\`!`
    );
...
router.get("/create", ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (parseInt(req.query.cpu) > maxLimitsResources.cpu)
      return res.redirect("../create-server?err=LIMITRESOURCES_CPU");
    if (parseInt(req.query.ram) > maxLimitsResources.memory)
      return res.redirect("../create-server?err=LIMITRESOURCES_RAM");
    if (parseInt(req.query.disk) > maxLimitsResources.disk)
      return res.redirect("../create-server?err=LIMITRESOURCES_DISK");

    if (parseInt(req.query.cpu) < minLimitsResources.cpu)
      return res.redirect("../create-server?err=LIMITRESOURCES_CPU");
    if (parseInt(req.query.ram) < minLimitsResources.memory)
      return res.redirect("../create-server?err=LIMITRESOURCES_RAM");
    if (parseInt(req.query.disk) < minLimitsResources.disk)
      return res.redirect("../create-server?err=LIMITRESOURCES_DISK");
    ...
    if (parseInt(req.query.ram) > parseInt(max.ram - existing.ram))
      return res.redirect("../create-server?err=NOTENOUGHRESOURCES");
    if (parseInt(req.query.disk) > parseInt(max.disk - existing.disk))
      return res.redirect("../create-server?err=NOTENOUGHRESOURCES");
    if (
      parseInt(req.query.database) > parseInt(max.database - existing.database)
    )
      return res.redirect("../create-server?err=NOTENOUGHRESOURCES");
    if (parseInt(req.query.backup) > parseInt(max.backup - existing.backup))
      return res.redirect("../create-server?err=NOTENOUGHRESOURCES");
    if (
      parseInt(req.query.allocation) >
      parseInt(max.allocation - existing.allocation)
    )
      return res.redirect("../create-server?err=NOTENOUGHRESOURCES");

    // Ensure resources are above 128MB / 10%
    if (parseInt(req.query.ram) < 128)
      return res.redirect("../create-server?err=INVALID");
    if (parseInt(req.query.cpu) < 10)
      return res.redirect("../create-server?err=INVALID");
    if (parseInt(req.query.disk) < 128)
      return res.redirect("../create-server?err=INVALID");
    ...
    if (
      isNaN(req.query.location) ||
      isNaN(req.query.egg) ||
      isNaN(req.query.cpu) ||
      isNaN(req.query.ram) ||
      isNaN(req.query.disk) ||
      isNaN(req.query.database) ||
      isNaN(req.query.backup) ||
      isNaN(req.query.allocation)
    )
      return res.redirect("../create-server?err=INVALID");
    if (req.query.cpu < 1 || req.query.ram < 1 || req.query.disk < 1)
      return res.redirect("../create-server?err=INVALID");
    ...
    await axios.post(
      `${provider.url}/api/application/servers`,
      {
        name: name,
        user: userId,
        egg: eggId,
...
router.get("/edit", ensureAuthenticated, async (req, res) => {
  ...
  if (
    ...
    return res.redirect("../dashboard?err=MISSINGPARAMS");
  ...
  try {
    ...
    if (parseInt(req.query.cpu) > limitsResources.cpu)
      return res.redirect("../edit-server?err=LIMITRESOURCESCPU");
    if (parseInt(req.query.ram) > limitsResources.memory)
      return res.redirect("../edit-server?err=LIMITRESOURCESRAM");
    if (parseInt(req.query.disk) > limitsResources.disk)
      return res.redirect("../edit-server?err=LIMITRESOURCESDISK");

    if (parseInt(req.query.cpu) > parseInt(max.cpu))
      return res.redirect("../edit-server?err=NOTENOUGHRESOURCES");
    if (parseInt(req.query.ram) > parseInt(max.ram))
      return res.redirect("../edit-server?err=NOTENOUGHRESOURCES");
    if (parseInt(req.query.disk) > parseInt(max.disk))
      return res.redirect("../edit-server?err=NOTENOUGHRESOURCES");
    if (parseInt(req.query.database) > parseInt(max.database))
      return res.redirect("../edit-server?err=NOTENOUGHRESOURCES");
    if (parseInt(req.query.backup) > parseInt(max.backup))
      return res.redirect("../edit-server?err=NOTENOUGHRESOURCES");
    if (parseInt(req.query.allocation) > parseInt(max.allocation))
      return res.redirect("../edit-server?err=NOTENOUGHRESOURCES");
    ...
    await axios.patch(
      `${provider.url}/api/application/servers/${serverId}/build`,
      {
        limits,
        feature_limits,
...
router.get("/edit-server", ensureAuthenticated, async (req, res) => {
  try {
    ...
    const userId = user.id;
    const server = await axios.get(
      `${provider.url}/api/application/servers/${req.query.id}`,
      {
        headers: {
          Authorization: `Bearer ${provider.key}`,
          Accept: "application/json",
